// =================== DASHBOARD.JS (Backend Integrated, Auto Refresh Token + Reports) ===================
    ...
    listContainer.innerHTML = `
            <div class="expense-list">
                ...
                <ul>
                    ${expenses.map(e => `
                        <li>
                            <span class="expense-date">${new Date(e.expenseDate).toLocaleDateString()}</span>
...
async function loadCategoryChart() {
  try {
    const res = await fetchWithAuth("http://localhost:8080/api/reports/expenses-by-category");
    ...
    const colors = [
      ...
      "#1abc9c", "#34495e", "#d35400", "#7f8c8d", "#27ae60",
...
async function loadMonthlyChart() {
  try {
    const res = await fetchWithAuth("http://localhost:8080/api/reports/monthly-expenses");
...
async function loadCategoryList() {
  try {
    ...
    categoryList.innerHTML = categories.map(cat => `<li>${cat}</li>`).join("");
  } catch (err) {
    notify(err.message, "error");
  }
}
...
async function renderSettings() {
  ...
  const profilePic = document.querySelector(".profile-container .user-profile img");
...
function updateProfileDisplay(userInfo) {
  const profilePicDisplay = document.querySelector('.profile-container .user-profile img');
  const usernameDisplay = document.querySelector('.profile-container .user-info .username');
  const emailDisplay = document.querySelector('.profile-container .user-info .email');
...
async function loadSummary() {
  try {
    const res = await fetchWithAuth("http://localhost:8080/api/expenses/summary");
...
async function loadTransactions() {
  try {
    ...
    const currentPageNumber = data.number + 1;
